...
def test_prepare_container_hooks_ignores_stale_managed_file_without_host_file(
    ...
    """Leave a stale managed hooks file unused when the host legacy file is absent."""
    managed_dir = tmp_path / "forge"
    managed_dir.mkdir()
    (managed_dir / "hooks.json").write_text("{}", encoding="utf-8")
...
def test_prepare_container_hooks_warns_and_falls_back_for_invalid_json(
    ...
    """Warn and generate an empty wrapped legacy file when the host JSON is invalid."""
    hooks_path = tmp_path / "hooks.json"
...
def test_prepare_container_hooks_warns_and_falls_back_for_unexpected_json_shape(
    ...
    """Warn and generate an empty wrapped legacy file when the host JSON is not an object."""
    hooks_path = tmp_path / "hooks.json"
