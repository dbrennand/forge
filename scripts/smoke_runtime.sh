...
docker run --rm \
  ...
  "${image}" \
  /bin/bash -lc 'test "$(id -un)" = "forge" && test "$HOME" = "/home/forge" && test "$CODEX_HOME" = "/home/forge/.codex" && command -v bwrap >/dev/null && command -v rg >/dev/null && touch /workspace/smoke-owned'
